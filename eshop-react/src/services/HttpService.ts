...
export default class DefaultHttpService implements HttpService {
  ...
  private getCredentials = (headers?: ApiHeader): RequestCredentials => {
    return headers && !!headers.authorization ? this.headerValueCredentialsTypeInclude : this.headerValueCredentialsTypeOmit;
  };
  ...
  private getHeaders = (headers?: ApiHeader): Record<string, string> => {
    ...
    if (headers) {
      if (headers.contentType !== undefined) {
        headersRequest = {
          ...headersRequest,
          [this.headerKeyContentType]: headers.contentType === ContentType.Json ? this.headerValueContentTypeJson : this.headerValueContentTypeFormData,
        };
      }
